...
/**
 * Simple Consumer sample. Start this class first, then run the producer.
 */
object MyConsumer extends App {
  ...
  val conn = system.actorOf(ConnectionOwner.props(connFactory, 1 second), "Connection")
  ...
  val consumer = ConnectionOwner.createChildActor(
    conn,
    Consumer.props(listener, channelParams = None, autoack = false),
